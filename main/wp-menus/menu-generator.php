...
class ContentCerberusMenuGenerator {
    ...
    public function get_menu($menu_location, $language) {
        ...
        foreach ($menus as $menu) {
            $menu = $this->rawb_api_map_fields($menu);
        ...
        $response->languages = array(
            '__autopopulate' => true,
            '__reference' => '/wp-json/rawb/v1/languages/' . $language,
        );
    ...
    function parse_item($menu) {
        ...
        if (isset($menu->object_id)) {
            $menu->page = array(
                '__autopopulate' => true,
                '__reference' => rtrim($menu->url, '/'), //$menu->object_id . '',
                '__include' => array('featured_info'),
            );
        }
    ...
    function rawb_api_map_fields($menu) {

        if($menu->children){
            foreach($menu->children as $child) {
                $child = $this->rawb_api_map_fields($child);
            }
        }

        // we should not have a description if no icon
        $menu_item_icon = get_field('menu_item_icon', $menu);
        if ( $menu_item_icon) {
            $menu->menu_item_icon = $menu_item_icon;

            $menu_item_description = get_field('menu_item_description', $menu);
            if ($menu_item_description) {
                $menu->menu_item_description = $menu_item_description;
            }
        }
